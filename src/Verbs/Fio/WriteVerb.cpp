...
int FioWriteVerb::performLineAppend(std::vector<std::string>& lines, const std::vector<std::string>& new_content, const LineRange& range) {
	...
	switch (range.type) {
		...
		case LineRange::FromEnd: {
			// @e:-3 - append after the last 3 lines (at end)
			insert_pos = lines.size();
			break;
		}
...
bool FioWriteVerb::shouldPreview(const std::string& path, const LineRange& range, WriteMode::Type write_mode, 
    ...
    if (preview_escape != content) {
        should_preview = true;
        preview_reason = "unicode_escaping";
    }
    
    // 2. Complex line surgery operations (NEW: Phase C1)
    // Au to Au: Consistency improvement - ALL line surgery operations should trigger previews
    if (range.type != LineRange::Full) {
        should_preview = true;
        if (preview_reason.empty()) preview_reason = "complex_line_surgery";
        else preview_reason += ",complex_line_surgery";
    }
    
    // 3. Large file operations (NEW: Phase C1)
    if (std::filesystem::exists(path)) {
        auto file_size = std::filesystem::file_size(path);
        if (file_size > 50000) { // 50KB threshold
            should_preview = true;
            if (preview_reason.empty()) preview_reason = "large_file";
            else preview_reason += ",large_file";
        }
    }
    
    // 4. Multi-line content operations (NEW: Phase C1)
    size_t newline_count = std::count(content.begin(), content.end(), '\n');
    if (newline_count > 10) {
        should_preview = true;
        if (preview_reason.empty()) preview_reason = "multi_line_content";
        else preview_reason += ",multi_line_content";
    }
    
    // 5. Destructive operations (NEW: Phase C1)
    if (write_mode == WriteMode::Replace && range.type != LineRange::SingleLine) {
        should_preview = true;
        if (preview_reason.empty()) preview_reason = "destructive_operation";
        else preview_reason += ",destructive_operation";
    }
