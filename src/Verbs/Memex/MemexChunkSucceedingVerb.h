...
//-----------------------------------------------------------------------------
// MemexChunkSucceedingVerb - Navigate to Next Chunk
//-----------------------------------------------------------------------------

/// Navigate to the chunk immediately succeeding the current chunk
/// Core component of Bush's associative navigation system
class MemexChunkSucceedingVerb : public IDb9Verb {
public:
    std::string getVerbName() const override { return "memex-chunk-succeeding"; }
...
private:
    ...
    /// Calculate succeeding chunk position and bounds
    MemexUtils::ChunkBounds calculateSucceedingBounds(const MemexUtils::ChunkInfo& current_chunk);
