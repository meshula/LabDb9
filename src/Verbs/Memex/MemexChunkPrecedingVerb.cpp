#include "MemexChunkPrecedingVerb.h"
...
std::string MemexChunkPrecedingVerb::getDescription() const {
    return R"Memex({
        "verb": "memex-chunk-preceding",
        "description": "Navigate to the chunk immediately preceding the current chunk (Bush's associative navigation)",
        "parameters": {
        ...
        "examples": [
            ...
            "(memex-chunk-preceding :chunk-id \"chunk_002847_24frags\" :include-text true :concatenate-text true :dbid db1)",
            "(memex-chunk-preceding :chunk-id \"chunk_002847_24frags\" :include-metadata false :dbid db1)"
        ],
        "response": {
...
MemexChunkPrecedingVerb::NavigationParameters MemexChunkPrecedingVerb::extractParameters(const lab::Text::Sexpr& sexpr) {
...
Db9Response MemexChunkPrecedingVerb::execute(const lab::Text::Sexpr& sexpr) {
    try {
        ...
        response.error_message = "memex-chunk-preceding execution failed: " + std::string(e.what());
        return response;
    }
}

Db9Response MemexChunkPrecedingVerb::performNavigation(const NavigationParameters& params) {
    ...
    auto preceding_bounds = MemexUtils::calculatePrecedingBounds(current_bounds);
    
    // Check if preceding chunk is possible
    if (preceding_bounds.start_position == 0 && current_bounds.start_position == 0) {
        Db9Response response;
        response.status = Db9Response::Error;
        response.error_message = "No preceding chunk available - already at beginning of rope";
        return response;
    }
    
    // Fetch entities for preceding chunk
    ...
    if (entities.empty()) {
        Db9Response response;
        response.status = Db9Response::Error;
        response.error_message = "No entities found in preceding chunk range";
        return response;
    }
    ...
    new_chunk.center_position = preceding_bounds.center_position;
    new_chunk.fragment_count = preceding_bounds.fragment_count;
...
std::string MemexChunkPrecedingVerb::formatNavigationResponse(
    ...
    if (params.include_metadata) {
        ...
        bool has_preceding = (preceding_bounds.start_position >= 0 && bounds.start_position > 0);
        bool has_succeeding = bounds.can_extend_forward;
