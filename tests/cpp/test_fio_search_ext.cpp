...
TEST(error_handling) {
    ...
    // Test missing path
    {
        ...
        if (response.status != Db9Response::Error) {
            std::cerr << "Should have reported a missing path parameter" << std::endl;
        }
        EXPECT_TRUE(response.status == Db9Response::Error);
    }
    ...
    // Test missing patterns
    {
        ...
        if (response.status != Db9Response::Error) {
            std::cerr << "Should have reported a missing patterns" << std::endl;
        }
        EXPECT_TRUE(response.status == Db9Response::Error);
    }
}
...
TEST(unicode_integration) {
    ...
    std::string sexpr_text = "(fio-search-ext :path testenv/unicode_test/sanskrit_mcguffins.md :patterns (स्वभाव svabhava) :case-fold true :ascii-fold true)";
    lab::Text::Sexpr sexpr(sexpr_text);
    
    FioSearchExtVerb verb;
    auto response = verb.execute(sexpr);
    
    std::cout << "Unicode response preview: " << response.result << std::endl;
